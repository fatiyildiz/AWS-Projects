# Project-503 : Blog Page Application (Django) deployed on AWS Application Load Balancer with Auto Scaling, S3, Relational Database Service(RDS), VPC's Components, DynamoDB and Cloudfront with Route 53 (DevOps_SOLUTION)

## Description

The Web Blog Page Application aims to deploy blog application as a web application written Django Framework on AWS Cloud Infrastructure. This infrastructure has Application Load Balancer with Auto Scaling Group of Elastic Compute Cloud (EC2) Instances and Relational Database Service (RDS) on defined VPC. Also, The Cloudfront and Route 53 services are located in front of the architecture and manage the traffic in secure. User is able to upload pictures and videos on own blog page and these are kept on S3 Bucket. This architecture will be created by Firms DevOps Guy.
...
### Step 1: Create dedicated VPC and whole components
    ...
    ### VPC
    - Create VPC. 
        create a vpc named `aws_web-blog-VPC` CIDR blok is `90.90.0.0/16` 
        no ipv6 CIDR block
        tenancy: default
    - select `aws_web-blog-VPC` VPC, click `Actions` and `enable DNS hostnames` for the `aws_web-blog-VPC`. 

    ## Subnets
    - Create Subnets
        - Create a public subnet named `aws_web-blog-public-subnet-1A` under the vpc aws_web-blog-VPC in AZ us-east-1a with 90.90.10.0/24
        - Create a private subnet named `aws_web-blog-private-subnet-1A` under the vpc aws_web-blog-VPC in AZ us-east-1a with 90.90.11.0/24
        - Create a public subnet named `aws_web-blog-public-subnet-1B` under the vpc aws_web-blog-VPC in AZ us-east-1b with 90.90.20.0/24
        - Create a private subnet named `aws_web-blog-private-subnet-1B` under the vpc aws_web-blog-VPC in AZ us-east-1b with 90.90.21.0/24
    ...
    - Click Internet gateway section on left hand side. Create an internet gateway named `aws_web-blog-IGW` and create.

    - ATTACH the internet gateway `aws_web-blog-IGW` to the newly created VPC `aws_web-blog-VPC`. Go to VPC and select newly created VPC and click action ---> Attach to VPC ---> Select `aws_web-blog-VPC` VPC 

    ## Route Table
    - Go to route tables on left hand side. We have already one route table as main route table. Change it's name as `aws_web-blog-public-RT` 
    - Create a route table and give a name as `aws_web-blog-private-RT`.
    - Add a rule to `aws_web-blog-public-RT` in which destination 0.0.0.0/0 (any network, any host) to target the internet gateway `aws_web-blog-IGW` in order to allow access to the internet.
...
Name            : aws_web-blog_EC2_Sec_Group
Description     : EC2 Security Groups only allows traffic coming from aws_web-blog_ALB_Sec_Group Security Groups for HTTP and HTTPS ports. In addition, ssh port is allowed from anywhere
VPC             : AWS_web-blog_VPC
Inbound Rules
HTTP(80)    ----> aws_web-blog_ALB_Sec_Group
HTTPS (443) ----> aws_web-blog_ALB_Sec_Group
ssh         ----> aws_web-blog_NAT_Sec_Group

4. RDS Security Groups
Name            : aws_web-blog_RDS_Sec_Group
Description     : EC2 Security Groups only allows traffic coming from aws_web-blog_EC2_Sec_Group Security Groups for MYSQL/Aurora port. 

VPC             : AWS_web-blog_VPC
Inbound Rules
MYSQL/Aurora(3306)  ----> aws_web-blog_EC2_Sec_Group
...
```text
Name               : aws_web-blog_RDS_Subnet_Group
Description        : aws web-blog RDS Subnet Group
VPC                : aws_web-blog_VPC
Add Subnets
Availability Zones : Select 2 AZ in aws_web-blog_VPC
...
Templates       : Free Tier
Settings        : 
    - DB instance identifier : aws-web-blog-RDS
...
Connectivity:
    VPC                      : aws_web-blog_VPC
    Subnet Group             : aws_web-blog_RDS_Subnet_Group
    Public Access            : No 
    VPC Security Groups      : Choose existing ---> aws_web-blog_RDS_Sec_Group
...
- Click Create Bucket
```text
Bucket Name : aws-web-blogs-<name>-blog
Region      : N.Virginia
Object Ownership
    - ACLs enabled
...
- Click Create Bucket
```text
Bucket Name : www.frankyildiz.com
Region      : N.Virginia
Object Ownership
    - ACLs enabled
        - Bucket owner preferred
...
## Step 5: Copy files downloaded or cloned from `frankyildiz` repo on Github 

## Step 6: Prepair your Github repository
- Create private project repository on your Github and clone it on your local. Copy all files and folders which are downloaded from frankyildiz repo under this folder. Commit and push them on your private Git hup Repo.
...
```text
- Movie and picture files are kept on S3 bucket named aws_web-blog_S3_<name>_Blog as object. You should create an S3 bucket and write name of it on "/src/cblog/settings.py" file as AWS_STORAGE_BUCKET_NAME variable. In addition, you must assign region of S3 as AWS_S3_REGION_NAME variable
...
```
- Please check if this userdata is working or not. to do this create new instance in public subnet and show to DevOps that it is working
...
Configure Instance Details  
    - Network : aws_web-blog_VPC
    - Subnet  : aws_web-blog-public-subnet-1A (Please select one of your Public Subnets)
    - Other features keep them as are
Storage ---> Keep it as is
Tags: Key: Name     Value: AWS web-blog NAT Instance
Configure Security Group
    - Select an existing security group: aws_web-blog_NAT_Sec_Group
...
Tags            : No tags
Role Name       : aws_web-blog_EC2_S3_Full_Access
...
```bash
Launch template name                : aws_web-blog_launch_template
Template version description        : Blog Web Page version 1
Amazon machine image (AMI)          : search `ubuntu` and select Ubuntu 18.04
Instance Type                       : t2.micro
Key Pair                            : mykey.pem
Security Groups                     : aws_web-blog_EC2_sec_group
Storage (Volumes)                   : keep it as is
Resource tags                       : Key: Name   Value: aws_web-blog_web_server
Advance Details:
    - IAM instance profile          : aws_web-blog_EC2_S3_Full_Access
...
Mappings                :
    - Availability zones:
        1. aws_web-blog-public-subnet-1A
        2. aws_web-blog-public-subnet-1B
Security Groups         :
Security Groups         : aws_web-blog_ALB_Sec_group

Listeners and Routing   :
Listener HTTPS:443
Protocol HTTP ---> Port 443 ---> Default Action Create Target Group (New Window pops up)
    - Target Type         : Instance
    - Name                : awsweb-blogTargetGroup
    - Protocol            : HTTP
    - Port                : 80
...
```text 
Auto Scaling group name         : aws_web-blog_ASG
Launch Template                 : aws_web-blog_launch_template
```
...
- Choose from your load balancer target groups  : awsweb-blogTargetGroup
- Health Checks
...
Create new Notification
    - Notification1
        - Send a notification to    : aws-web-blog-SNS
...
- Origin Settings
```text
Origin Domain Name          : aws-web-blog-ALB-1947210493.us-east-2.elb.amazonaws.com
...
Alternate Domain Names                  : www.frankyildiz.com
...
```text
Name                : aws web-blog health check
...
Health check            : aws web-blog health check
...
Choose: - LambdaS3fullaccess, 
        ...
        - DynamoDBFullAccess
No tags
Role Name           : aws_web-blog_lambda_Role
...
Go to the S3 console and select the S3 bucket named `awsweb-blogc3<name>blog`.

- Go to the properties menu ---> Go to the Event notifications part

- Click create event notification for creating object
```text
Event Name              : aws web-blog S3 event
Prefix                  : media/
Select                  :
    - All object create events
Destination             : Lambda Function
Specify Lambda function : Choose from your Lambda functions 
Lambda funstion         : awsweb-bloglambdafunction
click save
```text

```
- After create an event go to the `awsweb-bloglambdafunction` lambda Function and click add trigger on the top left hand side.

- For defining trigger for creating objects
```text
Trigger configuration   : S3
Bucket                  : awsweb-blogc3<name>blog
...
def lambda_handler(event, context):
    ...
    if event:
        ...
        dynamoTable = dynamo_db.Table('awsweb-blogDynamo')
...
- Congratulations!! You have finished your AWS web-blog Project